...
func (r *stockRepo) StockTransfer(ctx context.Context, req *models.StockTransfer) (int64, error) {
	query := `UPDATE stocks
SET quantity = quantity - $1
WHERE store_id = $2 AND product_id = $3  AND quantity >= $1;
`

	result, err := r.db.Exec(ctx, query, req.Quantity, req.StoreIdFrom, req.ProductId)
	if err != nil {
		return 0, err
	}
